...
-- Date:              Wed Dec 03 16:26:26 2014 (by Create and Import Peripheral Wizard)
...
architecture IMP of user_logic is
  ...
  signal wr_counter                      : UNSIGNED(C_SLV_DWIDTH-1 downto 0);
  signal fifo_full                       : std_logic;
  signal fifo_empty                      : std_logic;
  signal transfer_counter                : UNSIGNED(C_SLV_DWIDTH-1 downto 0);
  signal transfer_counter_next           : UNSIGNED(C_SLV_DWIDTH-1 downto 0);
  signal src_addr_counter                : UNSIGNED(C_SLV_DWIDTH-1 downto 0);
  signal src_addr_counter_next           : UNSIGNED(C_SLV_DWIDTH-1 downto 0);
  signal dst_addr_counter                : UNSIGNED(C_SLV_DWIDTH-1 downto 0);
  signal dst_addr_counter_next           : UNSIGNED(C_SLV_DWIDTH-1 downto 0);
  ------------------------------------------
  ...
  type CMD_CNTL_SM_TYPE is (CMD_IDLE, CMD_READ_RUN, CMD_WRITE_RUN, CMD_READ_WAIT_FOR_DATA, CMD_WRITE_WAIT_FOR_DATA, CMD_DONE);
...
begin
  ...
  --    Address Register    (C_BASEADDR + OFFSET + 0x4):
  ...
  --     Register      (C_BASEADDR + OFFSET + 0xC):
  --       bit 0-3  - Reserved
  ...
  -- 
  ------------------------------------------
  mst_reg_write_req <= Bus2IP_WrCE(6) or Bus2IP_WrCE(7) or Bus2IP_WrCE(8) or Bus2IP_WrCE(9);
  mst_reg_read_req  <= Bus2IP_RdCE(6) or Bus2IP_RdCE(7) or Bus2IP_RdCE(8) or Bus2IP_RdCE(9);
  mst_reg_write_sel <= Bus2IP_WrCE(9 downto 6);
  mst_reg_read_sel  <= Bus2IP_RdCE(9 downto 6);
  ...
  begin
    ...
    if ( Bus2IP_Clk'event and Bus2IP_Clk = '1' ) then
      if ( Bus2IP_Resetn = '0' ) then
        ...
        mst_cmd_sm_busy           <= '0';
        transfer_complete         <= '0';
        transfer_counter           <= TO_UNSIGNED(0, C_SLV_DWIDTH);
        src_addr_counter          <= UNSIGNED(slv_reg0);
        dst_addr_counter          <= UNSIGNED(slv_reg1);
      else
        ...
        mst_cmd_sm_set_timeout    <= '0';
        mst_cmd_sm_busy           <= '1';
        transfer_complete         <= '0';  --does this always make sure it's 0 unless complete?
        transfer_counter          <= transfer_counter;
        src_addr_counter          <= src_addr_counter;
        dst_addr_counter          <= dst_addr_counter;
        -- state transition
        case mst_cmd_sm_state is
          ...
          when CMD_READ_RUN =>
            if ( Bus2IP_Mst_CmdAck = '1' and Bus2IP_Mst_Cmplt = '0' ) then
              mst_cmd_sm_state <= CMD_READ_WAIT_FOR_DATA;
            elsif ( Bus2IP_Mst_Cmplt = '1') then  -- do another transfer
                transfer_counter <= transfer_counter_next;
                ...
                -- AXI4LITE data transfer error
                  mst_cmd_sm_set_error   <= '1';
                  mst_cmd_sm_state <= CMD_DONE;
                elsif(fifo_full = '1' or transfer_counter_next >= UNSIGNED(slv_reg2)) then
                    --increment the read address
                    src_addr_counter <= src_addr_counter_next;
                    --initialize the next write
                    mst_cmd_sm_state       <= CMD_WRITE_RUN;
                    mst_cmd_sm_rd_req      <= '0';
                    mst_cmd_sm_wr_req      <= '1';
                    mst_cmd_sm_ip2bus_addr <= STD_LOGIC_VECTOR(dst_addr_counter);
                    mst_cmd_sm_bus_lock    <= mst_cntl_bus_lock;
                else
                    --increment the read address
                    src_addr_counter <= src_addr_counter_next;
                    --initialize the next read
                    mst_cmd_sm_state <= CMD_READ_RUN;
                    mst_cmd_sm_rd_req      <= '1';
                    mst_cmd_sm_wr_req      <= '0';
                    mst_cmd_sm_ip2bus_addr <= STD_LOGIC_VECTOR(src_addr_counter_next);
                    mst_cmd_sm_bus_lock    <= mst_cntl_bus_lock;
                end if;
            else
              mst_cmd_sm_state       <= CMD_READ_RUN;
              mst_cmd_sm_rd_req      <= '1';
              mst_cmd_sm_wr_req      <= '0';
              mst_cmd_sm_ip2bus_addr <= STD_LOGIC_VECTOR(src_addr_counter);
              mst_cmd_sm_bus_lock    <= mst_cntl_bus_lock;
            end if;
           
          when CMD_WRITE_RUN=>
            if ( Bus2IP_Mst_CmdAck = '1' and Bus2IP_Mst_Cmplt = '0' ) then
                  mst_cmd_sm_state <= CMD_WRITE_WAIT_FOR_DATA;
                  ...
                  if ( Bus2IP_Mst_Cmd_Timeout = '1' ) then
                    ...
                    if(transfer_counter >= UNSIGNED(slv_reg2)) then
                        mst_cmd_sm_state <= CMD_DONE;
                    else
                        --increment the write address
                        dst_addr_counter <= dst_addr_counter_next;
                        --initialize the next read
                        mst_cmd_sm_state <= CMD_READ_RUN;
                        mst_cmd_sm_rd_req      <= '1';
                        mst_cmd_sm_wr_req      <= '0';
                    ...
                    mst_cmd_sm_rd_req      <= '0';
                    mst_cmd_sm_wr_req      <= '1';
                    mst_cmd_sm_ip2bus_addr <= STD_LOGIC_VECTOR(dst_addr_counter_next);
                    mst_cmd_sm_bus_lock    <= mst_cntl_bus_lock;
                end if;
            else
              mst_cmd_sm_state       <= CMD_WRITE_RUN;
         ...
         when CMD_WRITE_WAIT_FOR_DATA =>
          ...
          when CMD_DONE =>
            ...
            mst_cmd_sm_busy     <= '0';
            transfer_complete <= '1'; --indicates we've completed the DMA transfer
  ...
  IP2Bus_Error <= '0';
	...
	transfer_counter_next <= transfer_counter + 4;
    src_addr_counter_next <= src_addr_counter + 4;
    dst_addr_counter_next <= dst_addr_counter + 4;
end IMP;
